...
void setup()
{
  ...
  Serial.println("----------------------------------------");
  Serial.println(" Mesh relay v 0.0");
  Serial.println("----------------------------------------");
...
void loop()
{
    ...
    if (manager->available()) // There is something received
  {
    ...
    {
      // Packet not for us
      Serial.println("Not for us, relaying if needed");
      Serial.print("SNR = ");Serial.println(driver.lastSNR());
    }
    digitalWrite(LED_BUILTIN, LOW);
  }


  // if (millis() - lastMillis > 2222)
  // {
  //   // Xmit something to finish
  //   while (driver.isChannelActive())
  //   { 
  //     Serial.print("w");
  //   }

  //   Serial.print(millis()); Serial.print(" - TX: Mesh announce: [1] ");
  //   uint8_t returnValue = manager->sendtoWait(data, sizeof(data), 1);
  ...
  //     Serial.println("There was no route for dest 1 in the local routing table");
  //     break;
  //   case RH_ROUTER_ERROR_UNABLE_TO_DELIVER:
  //     Serial.println("Not able to deliver to the next hop (usually because it did not acknowledge due to being off the air or out of range");
  //     break;
  //   default:
  ...
  //   returnValue = manager->sendtoWait(data, sizeof(data), 2);
  //   Serial.print("\nTX: Mesh announce: [2] ");
  ...
  //     break;
  //   case RH_ROUTER_ERROR_NO_ROUTE:
  //     Serial.println("There was no route for dest 2 in the local routing table");
  //     break;
  //   case RH_ROUTER_ERROR_UNABLE_TO_DELIVER:
